...
def videoFrameHandler(event, sender, data):
    ...
    global IMG_HEIGHT, IMG_WIDTH, model, CATEGORIES
    frame = data.astype('float32') / 255
    frame = cv2.resize(frame, (IMG_WIDTH, IMG_HEIGHT))

    frame = frame.reshape(-1, IMG_WIDTH, IMG_HEIGHT, 3)
...
def handleFileReceived(event, sender, data):
    global date_fmt, counter
    # Create a file in ~/Pictures/ to receive image data from the drone.
    path = '%s/Pictures/LAND/land-%s.jpeg' % (
        os.getenv('HOME'),counter
...
def main():
    ...
    global model
    model = keras.models.load_model("./telloNetUncroppedwDenseDataset-1590495903.h5")
